...
public class AccountsJpaController implements Serializable {
    ...
    public void create(Accounts accounts) {
        ...
        try {
            ...
            // Retrieve the user ID based on the entered username
            Query query = em.createQuery("SELECT u.id FROM Users u WHERE u.username = :username");
            query.setParameter("username", accounts.getUsername());
            Integer userId = (Integer) query.getSingleResult();

            // Set the retrieved user ID into the accounts object
